...
For a categorized release log (Keep a Changelog format with date-anchored sections), see **[`../CHANGELOG.md`](../CHANGELOG.md)**. The list below is the deeper-context narrative for the same shipping moments.

## Recent milestones

- **2026-06-19 → 06-20 — Session token-spend panel + analyze/trail hardening.** New per-login-session token accounting: a `token_usage` Postgres table (auto-created by the release migration) keyed by an `hz_sid` cookie, captured at *both* OpenAI-compatible agent loops (`lib/llm/heroku.ts` and `lib/inference/heroku.ts`) via `stream_options.include_usage` with a char/4 estimate fallback. `GET /api/usage` (`app/api/usage/route.ts`) sums the session's rows grouped by model; `SessionUsageProvider` + `TokenSpendPanel` render it — in-flow right-rail on Today (collapsed by default), cross-tab dock on `/ask` + `/analyze` — showing per-model input/output subtotals, grand total, estimated cost (`lib/llm/modelPricing.ts`), turns, tool calls, and latency. The Ask Bar live-bumps the panel from a new `usage_meta` SSE event; per-step token counts now thread to *every* reasoning trail (`iterationUsage`), not just the Ask Bar. Section cache key bumped v1→v2 so cached Today trails pick up the token events. Same window: Analyze charts/tables were intermittently missing — fixed by raising MiniMax extract budgets and stripping `troubleshootingInfo` before the 8 KB cap; reasoning-trail turn headers no longer show gaps (tool-less synthesis turns) or out-of-order sequences (now rendered off the sorted `iterationUsage` spine); and a transport-successful-but-not-found `analyze_data` answer ("couldn't find any field or metric for CSAT") no longer burns the turn-wide once-budget, so the model's corrective retry against the real "CSAT Trends" metric reaches Tableau. New regression scripts: `test:trail`, `test:analyze-budget`. Specs at [`docs/superpowers/specs/2026-06-19-session-token-spend-design.md`](./superpowers/specs/2026-06-19-session-token-spend-design.md) + [plan](./superpowers/plans/2026-06-19-session-token-spend.md).
- **2026-05-12 — Mobile responsive Phase 0 + Next 15.5.18 bump.** `/analyze` and `/ask` were dead-end pages on phones — both workspace shells (`AnalyzeWorkspace.tsx`, `AskWorkspace.tsx`) hid the model list / thread list sidebars below `lg` (1024 px) with no mobile alternative, and the desktop `LeftRail` reserved 64 px (~17 % of a 375 px viewport) via an always-on `pl-16`. Fix: new shared `components/horizon/mobile/MobileDrawer.tsx` primitive (focus-trap, Esc, scroll-lock, backdrop tap), small per-surface client wrappers `AnalyzeMobileSidebar.tsx` + `AskMobileSidebar.tsx` that host the existing list components unchanged, `LeftRail` gated to `lg+` with `pl-16` → `lg:pl-16`, and `MobileNav.tsx` rewritten as a 3-icon section nav (Today / Ask / Analyze) replacing the prior 2-button scroll pill — mounted globally in `app/(banker)/layout.tsx` so all three surfaces get it. `ModelList.tsx` + `ThreadList.tsx` grow an optional `onSelect` callback so the drawer closes after navigation. Tailwind gains a `slide-in-left` keyframe mirroring the existing `slide-in-right`. Spec at `docs/superpowers/specs/2026-05-12-mobile-responsive-design.md`. Same day: `next` 15.5.15 → 15.5.18 patches 13 GHSA advisories (SSRF 8.6, middleware bypass 8.1, multiple DoS, XSS, cache poisoning); 6 transitive alerts remain (mcp-sdk hono/fast-uri/ip-address/express-rate-limit + Next-vendored postcss) — all gated on upstream releases, none runtime-reachable in this app's code paths (we use only mcp-sdk's `/client/` exports).
