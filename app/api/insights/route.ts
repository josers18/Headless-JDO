...
/**
 * C-1 — SectionInsight banner. Each major surface can POST here with
 * { section: "priority" | "pulse" | "drafts" | "signals" } and receive
 * a streaming agent response that produces a ≤ 24-word editorial banner.
 */
...
export async function POST(req: NextRequest) {
  ...
  const section =
    typeof (body as { section?: unknown })?.section === "string"
      ? ((body as { section: string }).section as SectionKind)
      : undefined;
