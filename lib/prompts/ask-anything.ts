...
3. Facet dispatch. Based on the category mapping above, fire the tool(s) that actually answer the question. For lookalike / anomaly / engagement questions on data_360: check if the DATA CLOUD CATALOG block is present in the system prompt — if yes, pick a DMO name VERBATIM from that catalog and call the Data Cloud SQL tool directly (the metadata tool has been filtered); if absent, skip the DC facet. For AUM / pipeline / win-rate questions on tableau_next: pick an apiName VERBATIM from the TABLEAU NEXT SEMANTIC MODELS catalog block in the system prompt (e.g. Transactions, Financial_Accounts, Trades) and call the analyze tool directly — never use "Sales"/"Service" as the model id, those are category labels not apiNames. The list/metadata discovery tools have been filtered out of this turn's tools array.
