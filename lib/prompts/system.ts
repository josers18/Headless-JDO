...
export const SYSTEM_PROMPT_VERSION = "v1.6.1-no-now-fn-soql-2026-05-08";
...
B. salesforce_crm SOQL:
   ...
   8. **No SQL-style functions in SOQL (recorded MALFORMED_QUERY: "unexpected token: NOW"):** SOQL is NOT SQL — \`NOW()\`, \`CURRENT_TIMESTAMP\`, \`GETDATE()\`, \`SYSDATE\`, etc. are not valid tokens and yield "unexpected token" parser errors. For "right now" on a DateTime column, omit the comparison and ORDER BY the column instead, or use the SOQL date-token forms above (\`TODAY\`, \`THIS_WEEK\`, \`LAST_N_DAYS:n\`). Date math like \`ActivityDate >= TODAY - 1\` is also invalid; use \`ActivityDate >= YESTERDAY\` or \`ActivityDate = LAST_N_DAYS:1\`.
